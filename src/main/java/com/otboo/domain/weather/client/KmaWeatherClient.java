...
public class KmaWeatherClient {
    ...
    public Mono<List<VilageFcstItem>> getForecast(int nx, int ny, VilageFcstBaseTime baseTime) {
        ...
        log.debug("기상청 예보 조회 요청 시작: nx={}, ny={}, baseDate={}, baseTime={}", nx, ny, baseDate, baseTimeValue);
        ...
        return webClient.get() // 요청 보내기 (여기선 아직 아무것도 안 감. 티켓만 만들어짐)
                ...
                .onErrorMap(
                        Exception.class,
                        e -> new KmaApiException(nx, ny, baseTime, e)
                )
                // 응답이 아예 안 온(빈 바디) 경우도 있어서 null 체크 대신 switchIfEmpty로 처리
                .switchIfEmpty(Mono.defer(() ->
                        Mono.error(new KmaApiException(nx, ny, baseTime, null))
                ))
                .flatMap(response -> {
                    if (response.response() == null
                        ...
                        return Mono.<List<VilageFcstItem>>error(
                                new KmaApiException(nx, ny, baseTime, null)
                        );
                    }
                    ...
                    log.debug("기상청 예보 조회 완료: nx={}, ny={}, baseDate={}, baseTime={}, count={}",
