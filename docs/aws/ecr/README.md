...
최종 자동 Push 대상 Branch는 `develop`입니다.
...
`otboo/backend` Repository에 이미지를 Push하는 데 필요한 권한만 부여합니다.
...
`otboo/backend` Repository에 제한하는 이미지 조회·Push 권한:
...
나머지 이미지 조회·업로드 권한은 `otboo/backend` Repository ARN으로 제한합니다.
...
Workflow 성공 후 ECR에서 Git Commit SHA 태그가 생성되었는지 확인합니다.

```
aws ecr describe-images \
  --repository-name "otboo/backend" \
  --image-ids imageTag="{Git Commit SHA}" \
  --region ap-northeast-2 \
  --profile otboo \
...
Issue #124에서는 위 세 값이 동일함을 확인했습니다.

또한 ECR에서 조회한 Image Digest와  
Pull 과정에서 확인한 Digest가 동일함을 확인했습니다.
...
- Issue #124: GitHub Actions OIDC 기반 이미지 빌드 및 ECR Push 구성 완료
